...
The HH team and its agency had a prior content optimisation exercise to annotate articles for combine and rewrite. The exercise was conducted manually, and was documented in an excel file which is used as a point of reference for the below study.

### Ablation Study on Pooling Strategy

First, an ablation study on pooling strategy to identify the best method to convert a sequence of embeddings into a single fixed-length representation of each article. In the context of embeddings, mean pooling and max pooling are two common strategies used to aggregate a set of embeddings (typically vectors) into a single embedding that represents a larger text segment or document.
...
In the ablation study, mean and max pooling strategies were evaluated using 3 Sentence Bert models (`all-MiniLM-L6-v2`, `all-mpnet-base-v2`, `msmarco-bert-base-dot-v5`) on a subset of the reference excel file provided by HH team (20 articles). 20 x 20 confusion matrices (heatmaps) were generated and evaluated. The figure below shows an example output using`msmarco-bert-base-dot-v5` embeddings with mean pooling. The text color-coded on each axis signifies that article titles of the same color belong to a single group of similar articles. Ideally, confusion matrixes should form along the diagonal axis in lighter colours (high correlation), while the areas away from the diagonal line should be as dark as possible (low correlation).

![Confusion Matrix / Heat Map generated from Dot Product Similarity between documents on `msmarco-bert-base-dot-v5` embeddings with mean pooling.](./img/heatmap.png)
...
2. **Median difference:** Difference in correlation scores between the median of same-group articles and that of different-group articles. This is a quantitative measure of intra-cluster versus inter-cluster similarity.
...
2. **Cluster Topic Specificity:** Assessing whether the topics represented by the clusters are too broad or if they can be refined into more specific sub-topics. This step helps in determining if the clusters are sufficiently detailed or if they require further segmentation to enhance their relevance and clarity.
...
`nomic-embed-text-v1.5` and `all-MiniLM-L6-v2` embedding models were able to generate specific-themed clusters not see in other models. 

### Conclusion

In conclusion, this study evaluated both pooling strategies and various embedding methods to determine the most effective approach for generating document embeddings. Key findings:

- Mean pooling outperforms max pooling in producing more coherent document representations, capturing the general meaning of the text better.
- Embedding models `nomic-embed-text-v1.5` and `all-MiniLM-L6-v2` generated more meaningful, specific-themed clusters, outperforming others in both quantitative and qualitative aspects.

In the downstream studies, experimentation were conducted using mean pooling strategy and embedding models `nomic-embed-text-v1.5` and `all-MiniLM-L6-v2` .